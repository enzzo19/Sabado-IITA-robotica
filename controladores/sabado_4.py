    ...
    if estado == 'deteccion':
        ...
        try: 
            if deteccion_izq[0] < 20:
                img_deteccion = camera_izq.getImage()
                a = 0
                start = robot.getTime()
                estado = 'clasificacion'
                print("CAMBIO AL ESTADO clasificaion")
            else:
                print(f'{deteccion_izq}, {deteccion_der}')
        except:
            pass
        try:
            if deteccion_der[0] > 20 and deteccion_der[0] != 0:
                img_deteccion = camera_der.getImage()
                a = 0
                start = robot.getTime()
                estado = 'clasificacion'
                print("CAMBIO AL ESTADO clasificaion")
    ...
    if estado == 'clasificacion':
        ...
        if robot.getTime() >= start + 1:
            if a == 0:
                ...
                img = np.array(np.frombuffer(img_deteccion, np.uint8).reshape((camera_centro.getHeight(), camera_centro.getWidth(), 4)))
